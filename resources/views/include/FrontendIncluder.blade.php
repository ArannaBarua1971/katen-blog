...
<div class="site-wrapper">
	...
	{{-- header content --}}
    @include('include.helper.frontendNav')
	{{-- header content end --}}
...
<div class="search-popup">
	...
	<div class="search-content">
		...
		<div class="searchOption">
			<ul id="searchOption" class="d-block" style="list-style-type: none">
			</ul>
		</div>
	</div>
</div>
		...
		$('#searchInput').on('keyup',function(){
			...
			$.ajax({
				...
				success:function($datas){
					let newLayoutForSearch=[];
					$datas.forEach(data => {
						let route="{{ route('postDeatails.all',':slug')}}"
						route=route.replace(':slug',data.slug);
						let list=`<li style="padding:5px"><a href="${route}">${data.title}</a></li>`;
						newLayoutForSearch.push(list);
					});
					$('#searchOption').html(newLayoutForSearch);
				}
			})
		})
	})
